...
contract DeployCoreSupala is Script, SelectRpc, Helper {
    ...
    function _getUtils() internal override {
        ...
        if (block.chainid == 1001) {
            ...
            supalaEmitter = SupalaEmitter(KAIA_TESTNET_SUPALA_EMITTER);
        } else {
            revert("Invalid chainid");
        }
    }

    function _deployMockToken(string memory _name) internal virtual returns (address) {
        if (keccak256(abi.encodePacked(_name)) == keccak256(abi.encodePacked("USDT"))) {
            mockUsdt = new MOCKUSDT();
            console.log("address public constant %s_MOCK_USDT = %s;", chainName, address(mockUsdt));
            return address(mockUsdt);
        } else if (keccak256(abi.encodePacked(_name)) == keccak256(abi.encodePacked("WKAIA"))) {
            mockWkaia = new MOCKWKAIA();
            console.log("address public constant %s_MOCK_WKAIA = %s;", chainName, address(mockWkaia));
            return address(mockWkaia);
        } else if (keccak256(abi.encodePacked(_name)) == keccak256(abi.encodePacked("WETH"))) {
            mockWeth = new MOCKWETH();
            console.log("address public constant %s_MOCK_WETH = %s;", chainName, address(mockWeth));
    ...
    function _deployOft() internal virtual {
        elevatedminterburner = new ElevatedMinterBurner(usdt, owner);
        console.log("address public constant %s_USDT_ELEVATED_MINTER_BURNER = %s;", chainName, address(elevatedminterburner));
        oftadapter = new OFTadapter(usdt, address(elevatedminterburner), endpoint, owner, _tokenDecimals(usdt));
        console.log("address public constant %s_USDT_OFT_ADAPTER = %s;", chainName, address(oftadapter));
        ...
        console.log("address public constant %s_WKAIA_ELEVATED_MINTER_BURNER = %s;", chainName, address(elevatedminterburner));
        oftadapter = new OFTadapter(wNative, address(elevatedminterburner), endpoint, owner, _tokenDecimals(wNative));
        console.log("address public constant %s_WKAIA_OFT_ADAPTER = %s;", chainName, address(oftadapter));
        oftNativeAdapter = address(oftadapter);
        oapp2 = address(oftadapter);
        elevatedminterburner.setOperator(oapp2, true);

        elevatedminterburner = new ElevatedMinterBurner(weth, owner);
        console.log("address public constant %s_WETH_ELEVATED_MINTER_BURNER = %s;", chainName, address(elevatedminterburner));
        oftadapter = new OFTadapter(weth, address(elevatedminterburner), endpoint, owner, _tokenDecimals(weth));
        console.log("address public constant %s_WETH_OFT_ADAPTER = %s;", chainName, address(oftadapter));
        oftNativeOriAdapter = address(oftadapter);
        oapp3 = address(oftadapter);
        elevatedminterburner.setOperator(oapp3, true);
    }

    function _setLibraries() internal virtual {
    ...
    function _setSendConfig() internal virtual {
    ...
    function _setReceiveConfig() internal virtual {
        UlnConfig memory uln = UlnConfig({
            confirmations: 15,
            requiredDVNCount: block.chainid == 1001 ? 1 : 2,
            optionalDVNCount: type(uint8).max,
    ...
    function _deployTokenDataStream() internal virtual {
        ...
        tokenDataStream.setTokenPriceFeed(weth, ETH_USDT);
    }
    ...
    function _setFactoryConfig() internal virtual {
        ...
        IFactory(address(lendingPoolFactory)).setMinAmountSupplyLiquidity(weth, 0.00005 ether);
    }
    ...
    function _setInterestRateModelTokenReserveFactor() internal virtual {
